    ...
    <ul id="sortable" class="ui-sortable sortable" rel="cosa">
        <?php foreach ($this->modelsPackage as $key => $value) { ?>
            <li id="foo_<?= $value['model_id']; ?>" class="ui-state-default listImage modelList" onclick="">
                ...
                <p><?=$value['name']?></p>
                <a href="<?=WEB?>ES/gallery/model/<?=$value['model_id']?>" target="_blank"><input class="btnSmall editImg" type="button" value="View Portfolio"></a>
                <a href="<?=URL . LANG?>/models/editportafolio/<?= $value['model_id']; ?>" target="_blank"><input class="btnSmall editImg" type="button" value="Edit Portfolio"></a>
