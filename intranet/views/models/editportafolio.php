    ...
    <div>
        ...
        <ul id="sortable" class="ui-sortable sortable modelListImages" rel="cosa">
            ...
            foreach ($this->modelPhotos as $key => $value) {
                ...
                ?>
                <li id="foo_<?=$value['id']; ?>" class="ui-state-default modelList <?= ($value['main']) ? 'mainPic' : '' ?>" onclick="">
                    <input value="<?=$value['id']; ?>" name="check[]" class="checkFoto" type="checkbox">
                    <img width="154" height="207" class="listImage" caption="<?=$value['caption_' . LANG]; ?>" src="<?= URL . UPLOAD . 'models/' . Model::idToRute($value['photo_id']) . 'thumb_' . $value['file_file_name'].$strNoCache; ?>"/>
                    ...
                    <a target="_blank" href="<?= URL . UPLOAD . 'models/' . Model::idToRute($value['photo_id']) . $value['file_file_name']; ?>"><input id="h1096" class="btnSmall" type="button" value="View" ></a>
