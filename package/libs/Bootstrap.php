...
class Bootstrap {
    ...
    private function _getUrl()
    {
        ...
        $PACKAGE=array_shift($this->_url);
        $TYPEBOOKING=$this->_url[0];
        if(!$TYPEBOOKING) 
            $TYPEBOOKING='selection';
        define('TYPEBOOKING',$TYPEBOOKING);
        $PACKAGE=str_replace('+','-',$PACKAGE);
        define('PACKAGE',$PACKAGE);
        $package=explode('-',PACKAGE);
        define('PACKAGE_ID',array_shift($package));
        define('DELIVER_ID',array_shift($package));
